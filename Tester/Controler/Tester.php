...
class Tester extends ControllerAbstract {
     ...
     * Zapíše do repository (Uloží) agregatni strukturu (objekt) ...AggragateEntity.
     ...
     * 
     * Pozn.: Nastavuje testUkoncen = false  a  testZahajen=true  v agregatnim objektu ...AggragateEntity,
...
//            $getTestAggregateEntity->zadaniTestuAggregate = $zadaniTestuAggregateEntity;
              ...
              $testAggregateEntity = new AggEntity\TestAggregate( ) ;    // nove , nutno naplnit    
              $testAggregateEntity->prubehTestu->uidKonfiguraceTestuFk = $uidKonfiguraceTestu; // to uz vim, nezavisi na ukladani do db
              $testAggregateEntity->prubehTestu->casSpusteni =  new \DateTime(); 
              $testAggregateEntity->prubehTestu->poleNavic =  "Něco do pole navic2.";   
  
              $testAggregateEntity->sessionTestu->testUkoncen = \FALSE;          // ***  NASTAVUJE *** pro "novy" prubeh
              $testAggregateEntity->sessionTestu->testZahajen = \TRUE;           // ***  NASTAVUJE *** pro "novy" prubeh  
              
               /* @var $dataAggregateEntity AggEntity\DataAggregate */
               $dataAggregateEntity = new AggEntity\DataAggregate( ) ;    // nove , nutno naplnit  
              
               $dataAggregateEntity->ticketPouzity->identifikatorTicketu = $identifikatorTicketu;
               
               $dataAggregateEntity->testAggregate = $testAggregateEntity;
               
               $dataAggregateEntity->zadaniTestuAggregate = $zadaniTestuAggregateEntity;
                                   
               
            //----- !!! AGGREGAT ULOZIT  !!! ------            
            /* @var $dataAggregateRepository AggRepo\DataAggregateRepository */
            $dataAggregateRepository = $this->container->get( AggRepo\DataAggregateRepository::class  ) ;
            $dataAggregateRepository->add( $dataAggregateEntity );     // !!!  ULOZIT    !!!
...
//            //-------------------------------------------------------------------------------------------------------------------
            ...
            $poleProTemplate = [
                ...
                'nazevTestu' => $dataAggregateEntity->zadaniTestuAggregate->konfiguraceTestu->nazevTestu,
  /* ?? */      'casSpusteni'   => $dataAggregateEntity->testAggregate->prubehTestu->casSpusteni->format('Y-m-d H:i:s'), //datetime
                'idPrubehTestu' => $dataAggregateEntity->testAggregate->prubehTestu->idPrubehTestu,
                'vstupHtmlForm' => $html,      
...
//        $zadaniTestuAggregateEntity = $zadaniTestuAggregateRepository->getPodleUidKonfigurace( $getTestAggregateEntityPodleSession->prubehTestu->uidKonfiguraceTestuFk );                                         
    ...
    public function prejdiNaUlohu(  $idCilovaUloha) {      
        //$url = "http://localhost/TesterVScviceni/Tester/uloha/" .  $idCilovaUloha . "/";   
    ...
    public function saveOdpoved( $idUloha ) {                       
        ...
        if ($tabbed->isValid()) {      
            ...
            //$url = "http://localhost/TesterVScviceni/Tester/dekujeme/" ;    
  ...
  /* */          $odpovedAggRepo = $this->container->get( AggRepo\OdpovedAggregateRepository::class);  
            ...
            foreach ($vsechnyOdpovedi as $key=>$value) {   //v key jsou identifikatory
                $odpovedNaOtazku = new DbEntity\OdpovedNaOtazkuRow();  
...
$postTestAggregateRepository->add($postTestAggregateEntityPodleZSession);       //   U K L A D A M 
            ...
            if ($validStranka) {     //  - Current page is valid,  display jinou
                ...
                //$url = "http://localhost/TesterVScviceni/Tester/uloha/$idUlohaDalsi/"; 
                ...
                //$url = "http://localhost/TesterVScviceni/Tester/uloha/$idUloha/";
